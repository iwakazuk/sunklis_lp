...
export default function QuestionSection({
  ...
  const [selectedAnswer, setSelectedAnswer] = useState<DiagnosisAnswer | null>(currentAnswer || null);
      ...
      <div className="px-4 pt-4 pb-3 space-y-3">
        ...
        {selectedAnswer && (
          <UserMessage className="animate-slideInRight">
            <p className="text-sm leading-relaxed">{selectedAnswer.label}</p>
          </UserMessage>
        )}
      </div>

      {/* 選択肢エリア */}
      <div className="mt-auto px-4 pb-4 pt-2 border-t border-gray-100 bg-gray-50/50 animate-fadeIn">
            ...
            {fallbackPrefectureOption && (
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-xl border text-[15px] leading-[1.4] font-normal transition-all duration-200 ${
                  selectedAnswer?.id === fallbackPrefectureOption.id
